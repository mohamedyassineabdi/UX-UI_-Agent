...
DETECTOR_HEURISTIC_WEIGHTS = {
    ...
    "generic_offer_without_proof": {
        "heuristics": ["Match between system and the real world", "Aesthetic and minimalist design"],
        "weight": 0.82,
    },
}
DETECTOR_PRIORITY = {
    ...
    "generic_offer_without_proof": 10,
}
...
def _issue_pattern_key(issue: dict[str, object]) -> str:
    ...
    if detector_id in {"generic_navigation_label", "placeholder_or_generic_copy"}:
        ...
        subdetector = _clean_text(
            ...
            or evidence.get("market_alignment_subdetector")
            or ""
        )
...
def _issue_summary(issue: dict[str, object]) -> str:
    ...
    if detector_id == "generic_offer_without_proof":
        checks = _checks(evidence, "value_communication_checks")
        placeholder = _sentence_list(_evidence_list(checks, "placeholder_identity_hits"))
        sample = _quoted(evidence.get("text_sample") or evidence.get("visible_placeholder_identity"))
        if subdetector == "placeholder_offer_identity":
            return (
                f"In {area}, visible placeholder identity text is still present"
                f"{f' ({sample})' if sample else f' ({placeholder})' if placeholder else ''}. "
                "This is a concrete copy problem because the boxed text is still template content, not final user-facing content."
            )
        return (
            f"In {area}, a visible placeholder or generic identity label is still present. "
            "The report only flags this because the exact text can be boxed in the screenshot."
        )
...
def _why_it_matters(issue: dict[str, object], criterion: UxUiCriterion | None) -> str:
    ...
    if detector_id == "generic_offer_without_proof":
        if subdetector == "placeholder_offer_identity":
            return (
                "Placeholder identity text makes the design look unfinished and prevents reviewers from judging the real product name, section, or message."
            )
        return (
            "Users and clients need final visible copy to understand what the screen is saying. Template labels reduce clarity."
        )
    if criterion:
...
def _human_issue_title(issue: dict[str, object]) -> str:
    ...
    if detector_id == "generic_offer_without_proof":
        if subdetector == "placeholder_offer_identity":
            return "Placeholder identity text is visible"
        return "Visible identity copy is too generic"
...
def _recommendation(issue: dict[str, object], criterion: UxUiCriterion | None) -> str:
    ...
    if detector_id == "generic_offer_without_proof":
        if subdetector == "placeholder_offer_identity":
            return (
                "Replace the boxed placeholder with the real product, company, section, or feature name. "
                "Keep the wording specific enough that the screen makes sense without Figma layer names."
            )
        return (
            "Replace the boxed generic identity copy with final user-facing text."
        )
    if criterion:
