...
AXIS_CHECKS: tuple[AxisCheck, ...] = (
    # Axis 1: Form and task execution.
    ...
    # Axis 3: Trust, accessibility, readability, touch, and spacing.
    _check("trust_accessibility", "Primary content readability", "important", "Main titles, prices, body copy, statuses, and action labels must have enough rendered contrast against their background.", ("low_text_contrast",)),
    _check("trust_accessibility", "Decision label readability", "important", "Visible option labels, selected values, status text, and timing cues must stay readable when they affect a user decision.", ("low_text_contrast",)),
    _check("trust_accessibility", "Busy background readability", "important", "Text over images, gradients, or visually busy surfaces must remain readable in the captured screenshot.", ("low_text_contrast",), active=False, analysis_method="human_review"),
    _check("trust_accessibility", "Small important text", "important", "Important labels, buttons, prices, status, and instructions should not be too small to read on mobile.", ("small_text_readability",)),
    ...
    _check("trust_accessibility", "Comfortable mobile touch targets", "important", "Visible controls should be large enough to tap reliably on a phone.", ("small_touch_target",)),
    _check("trust_accessibility", "Crowded tap zones", "important", "Interactive controls should not sit so close together that accidental taps become likely.", ("crowded_touch_target",)),
    ...
    _check("trust_accessibility", "Icon-only action clarity", "important", "Icon-only controls need visible labels or unmistakable symbols when the meaning is not obvious.", ("icon_only_unlabeled_control",)),
    ...
    _check("trust_accessibility", "Visible active/focus state", "secondary", "Selected, active, or focused states should be visually clear enough to identify.", ("component_style_outlier",), active=False, analysis_method="human_review"),
    ...
    # Axis 4: UI consistency and component behavior.
    ...
    _check("visual_brand", "Clear visual hierarchy", "important", "Visible screens need a clear focal point and readable priority order.", ("flat_visual_hierarchy",)),
    _check("visual_brand", "Primary action dominance", "important", "A primary action should be visually dominant when the screen expects the user to act.", ("flat_visual_hierarchy",), active=False, analysis_method="human_review"),
    _check("visual_brand", "Competing element control", "important", "Too many similarly strong elements should not compete for first attention.", ("flat_visual_hierarchy",)),
    _check("visual_brand", "Visible starting point", "important", "The screen should have a clear first element to read or act on.", ("flat_visual_hierarchy",)),
    _check("visual_brand", "Important content placement", "important", "Important information should not be pushed too far from the main focus area.", ("flat_visual_hierarchy",), active=False, analysis_method="human_review"),
    _check("visual_brand", "Heading size contrast", "important", "Headings, subheadings, and body text should have visibly different priority.", ("flat_visual_hierarchy",)),
    _check("visual_brand", "Card priority clarity", "secondary", "Cards or sections should not look equally important when one carries the main decision.", ("flat_visual_hierarchy",), analysis_method="ai_assisted"),
    _check("visual_brand", "Decorative dominance", "secondary", "Decorative imagery or shapes should not visually overpower functional content.", ("flat_visual_hierarchy",), analysis_method="ai_assisted"),
    _check("visual_brand", "Status emphasis", "secondary", "Important feedback, warnings, or confirmations should be visually emphasized enough to notice.", ("flat_visual_hierarchy",), active=False, analysis_method="human_review"),
    _check("visual_brand", "Balanced first-glance composition", "secondary", "Large visible surfaces should not feel accidentally lopsided or visually heavy.", ("flat_visual_hierarchy",), analysis_method="ai_assisted"),

    # Axis 6: Content clarity and microcopy.
    ...
    _check("content_microcopy", "Scannable plain language", "secondary", "Visible text should be concrete, not vague, dense, or hard to scan on mobile.", ("placeholder_or_generic_copy",), analysis_method="ai_assisted"),
    ...
    # Axis 7: Visible offer and market wording. This axis is intentionally
    # constrained to boxable text; business strategy is not inferred.
    _check("market_alignment", "Visible identity placeholder check", "important", "Only visible placeholder identity text like Company Name, Product Name, App Name, or Tagline is flagged.", ("generic_offer_without_proof", "placeholder_or_generic_copy")),
    _check("market_alignment", "Boxable offer-copy clarity", "important", "Offer clarity is only reported when a specific visible word, title, CTA, or label can be boxed.", ("placeholder_or_generic_copy",)),
    _check("market_alignment", "Placeholder product or service label", "important", "Visible product, service, or company placeholders should be replaced by final copy.", ("generic_offer_without_proof", "placeholder_or_generic_copy")),
    _check("market_alignment", "Generic offer CTA", "important", "Visible commercial CTAs should explain their action when the CTA itself is the issue.", ("placeholder_or_generic_copy", "ambiguous_completion_action")),
    _check("market_alignment", "Unclear visible offer title", "important", "A visible offer title should not be Title, Heading, Product Name, or similarly generic copy.", ("placeholder_or_generic_copy",)),
    _check("market_alignment", "Repeated commercial placeholders", "important", "Repeated App Name, Product Name, Label, or Menu Item text should not make the offer look unfinished.", ("placeholder_or_generic_copy", "generic_offer_without_proof")),
    _check("market_alignment", "Truncated price or offer copy", "secondary", "Visible price, offer, or product text should not be clipped or ellipsized.", ("placeholder_or_generic_copy",)),
    _check("market_alignment", "Vague visible benefit phrase", "secondary", "A visible benefit phrase can be flagged only when the exact vague text can be boxed.", ("placeholder_or_generic_copy",), analysis_method="ai_assisted"),
    _check("market_alignment", "Incomplete visible template block", "secondary", "A visible offer block can be flagged only when a placeholder or incomplete element is boxed.", ("placeholder_or_generic_copy", "generic_offer_without_proof")),
    _check("market_alignment", "No assumption-only market judgment", "secondary", "Missing proof, audience, pricing, or strategy is not reported unless the problem is represented by a visible boxable UI element.", (), analysis_method="human_review", active=False),
)
