...
def _brand_evidence(subdetector: str, checks: dict[str, object]) -> dict[str, object]:
    return {
        "validation_method": "balanced_aesthetics_static_figma_gate",
...
def detect_flat_visual_hierarchy(normalized_file: NormalizedFigmaFile) -> list[AuditIssue]:
    """
    Detect large frames where hierarchy or visual balance weakens brand composition.

    These are lower-confidence visual-brand draft signals because brand meaning
    is contextual, but the measured font-size and visual-weight evidence is
    objective enough to guide human review.
    """
