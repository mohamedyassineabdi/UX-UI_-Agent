...
#     filtered = [clean_text(value) for value in values if clean_text(value)]
#     if not filtered:
#         return []
...
#     top_width = round_px_token(styles.get("borderTopWidth")) or "0px"
#     top_style = clean_text(styles.get("borderTopStyle")) or "none"
#     top_color = normalize_color(styles.get("borderTopColor")) or "transparent"
#     return f"{top_width} {top_style} {top_color}"


# def get_shadow_token(styles: Dict[str, Any]) -> str:
#     shadow = clean_text(styles.get("boxShadow"))
#     if shadow.lower() in {"", "none"}:
#         return ""
#     return shadow
...
#     return lowered in SKIP_LINK_TERMS


# def looks_like_nav_text(text: str) -> bool:
#     lowered = clean_text(text).lower()
#     return lowered in NAV_TERMS
...
#         return True
#     if clean_text(styles.get("cursor")).lower() == "pointer":
#         return True
#     if clean_text(element.get("href")):
#         return True
#     return False
...
#         return False
#     return likely_interactive(element)
...
#     if not text or text_length > 140:
#         return False
...
#     if any(token in class_name for token in ["title", "heading", "headline", "subtitle"]):
#         if font_size >= 16 or font_weight >= 600:
#             return True

#     if font_size >= 30 and font_weight >= 600 and text_length <= 90:
#         return True
#     if font_size >= 24 and font_weight >= 600 and text_length <= 100:
#         return True
#     if font_size >= 20 and font_weight >= 700 and text_length <= 110:
#         return True
#     if font_size >= 18 and font_weight >= 700 and text_length <= 80 and line_height and line_height <= font_size * 1.45:
#         return True
...
#     if any(token in class_name for token in ["card", "tile", "panel", "product", "pricing"]):
#         return True

#     if tag in {"section", "article", "div", "li"} and width >= 140 and height >= 80:
...
#     if tag in {"section", "article", "main", "aside"} and width >= 220 and height >= 80:
#         return True

#     if any(token in class_name for token in ["section", "hero", "banner", "footer", "header", "content", "container"]):
#         if width >= 220 and height >= 70:
#             return True

#     if tag == "div" and width >= 240 and height >= 80 and child_count >= 2:
...
#     if not meaningful_text and tag not in {
#         "input",
#         "select",
#         "textarea",
#         "form",
...
#             "link",
#         }:
#             if not looks_like_card_container(element) and not looks_like_section_container(element):
#                 return True
...
#     ]
#     if any(token in class_name for token in noisy_class_tokens) and not meaningful_text:
#         return True
...
#         variant = detect_component_variant(element, semantic_type)
#         style_signature = build_style_signature(element)
#         component_group_id = build_component_group_id(element)

#         enriched = dict(element)
#         enriched["semanticType"] = semantic_type
#         enriched["componentVariant"] = variant
#         enriched["styleSignature"] = style_signature
#         enriched["componentGroupId"] = component_group_id
...
#         font_family = normalize_css_value(styles.get("fontFamily"))
...
#         font_weight = normalize_css_value(styles.get("fontWeight"))
...
#             "counts": {
#                 "values": len(set(spacing_values)),
#             },
#         },
#         "layout": {
#             "modes": pick_top_values(layout_modes, 12),
#             "counts": {
#                 "modes": len(set(layout_modes)),
#             },
#         },
#     }
...
#         first = members[0]
#         rects = [member.get("rect") or {} for member in members]
#         widths = [safe_float(rect.get("width")) for rect in rects if safe_float(rect.get("width")) > 0]
#         heights = [safe_float(rect.get("height")) for rect in rects if safe_float(rect.get("height")) > 0]
...
#     families = [family for family in families if family["semanticType"] in MEANINGFUL_AUDIT_TYPES]
#     families.sort(key=lambda item: (-item["count"], item["semanticType"], item["componentVariant"]))
#     return families[:200]
...
#     hidden_items.sort(key=hidden_interactive_priority, reverse=True)
#     return hidden_items[:220]
...
#     forms.sort(key=lambda item: item["formKey"])
#     return forms[:120]
...
#     headings = [element for element in elements if clean_text(element.get("semanticType")) == "heading"]
...
#         if clean_text(family.get("semanticType")) in {"button", "button-ghost", "cta-link"}
#     ]
#     if len(button_families) > 5:
#         component_risks.append("Many distinct button or CTA style families detected.")

#     nav_families = [family for family in component_families if clean_text(family.get("semanticType")) == "nav-link"]
#     if len(nav_families) > 4:
#         component_risks.append("Many distinct navigation link style families detected.")

#     card_families = [family for family in component_families if clean_text(family.get("semanticType")) == "card"]
...
#             narrow_fields = [
#                 field for field in visible_fields
#                 if safe_float((field.get("rect") or {}).get("width")) < 140
#             ]
#             if len(narrow_fields) >= 2:
#                 layout_risks.append(
#                     f"Form '{form.get('formId') or form.get('formName') or form.get('formKey')}' contains multiple narrow fields that may affect usability."
#                 )
...
#     max_elements_per_page = safe_int(rendered_ui_config.get("maxElementsPerPage", 1600), 1600)
...
#     deduplicated_elements = unique_by_fingerprint(normalized_elements)[:max_elements_per_page]
#     audit_elements = build_audit_elements(deduplicated_elements)
#     hidden_interactive = build_hidden_interactive_inventory(deduplicated_elements)
...
#         "totalElements": len(deduplicated_elements),
